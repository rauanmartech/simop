...
export const MuseumCard: React.FC<MuseumCardProps> = ({ museum, className = "" }) => {
  return (
    <article className={`clay-card overflow-hidden flex flex-col h-full group ${className}`}>
      {/* Cover Image Container */}
      <div className="relative w-full h-56 md:h-60 overflow-hidden bg-stone/30">
        <img src={museum.imagem_capa} alt={museum.nome} className="absolute inset-0 w-full h-full object-cover transition-transform duration-700 group-hover:scale-105" />
      ...
      <div className="p-8 flex flex-col flex-grow justify-between gap-4">
        <div className="space-y-2">
          <h3 className="font-serif text-xl font-bold text-night group-hover:text-gold transition-colors leading-snug">
            <Link href={`/museus/${museum.slug}`} className="focus:outline-none">
              {museum.nome}
            </Link>
          </h3>
